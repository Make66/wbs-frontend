...
//    For example:
const name = firstItem<string>(["Ada", "Grace"]); // "Ada"
const number = firstItem<number>([10, 20, 30]); // 10
const none = firstItem<undefined>([]); // undefined
const mixed = firstItem<string | number | boolean>(["Ada", 1, false]);
//
//    Hint: return type should be T | undefined
console.log(name, number, none, mixed);
...
const firstName = pluck(user, "name"); // "Ada"
